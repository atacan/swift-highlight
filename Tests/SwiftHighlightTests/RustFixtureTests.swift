...
/// Tests Rust highlighting against the original highlight.js test fixtures
final class RustFixtureTests: XCTestCase {
    ...
    private func runFixture(_ name: String, file: StaticString = #file, line: UInt = #line) async {
        guard let inputURL = Bundle.module.url(forResource: name, withExtension: "txt", subdirectory: "Fixtures/rust"),
              let expectedURL = Bundle.module.url(forResource: "\(name).expect", withExtension: "txt", subdirectory: "Fixtures/rust") else {
        ...
        let result = await hljs.highlight(input, language: "rust")
    ...
    func testInvokedKeywords() async throws {
        await runFixture("invoked-keywords")
    }

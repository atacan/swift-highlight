...
/// Tests Go highlighting against the original highlight.js test fixtures
final class GoFixtureTests: XCTestCase {
    ...
    private func runFixture(_ name: String, file: StaticString = #file, line: UInt = #line) async {
        guard let inputURL = Bundle.module.url(forResource: name, withExtension: "txt", subdirectory: "Fixtures/go"),
              let expectedURL = Bundle.module.url(forResource: "\(name).expect", withExtension: "txt", subdirectory: "Fixtures/go") else {
        ...
        let result = await hljs.highlight(input, language: "go")
